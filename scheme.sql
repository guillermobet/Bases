...
CREATE TABLE AYUDANTE (
	...
	CREATE TABLE supervision (
		...
		CONSTRAINT PK_supervision PRIMARY KEY(cedula, supervision)
	);
...
ADD CONSTRAINT FK_AYUDANTE FOREIGN KEY(cedula, id_contrato) REFERENCES CONTRATO(cedula, id_contrato);
	...
	ADD CONSTRAINT FK_supervision FOREIGN KEY(cedula) REFERENCES AYUDANTE(cedula);

ALTER TABLE CPROFESOR 
ADD CONSTRAINT FK_CPROFESOR FOREIGN KEY(cedula, id_contrato) REFERENCES CONTRATO(cedula, id_contrato),
ADD CONSTRAINT DOM_TIPO CHECK ('Dedicación integral', 'Dedicación exclusiva', 'Dedicación convencional'),
ADD CONSTRAINT DOM_TIPO_HORAS CHECK ((tipo = "Dedicación convencional" AND (horas = 4 OR horas = 6 OR horas = 8 OR horas = 12))
										OR ((tipo = 'Dedicación exclusiva' OR tipo = 'Dedicación integral') AND horas > 0 AND horas < 11));
